...
U15_FIND=$(find / \( -nouser -o -nogroup \) -xdev -ls 2>/dev/null)

echo -e "\n==============================================================" >> $OUTPUT_FILE
echo "U15.소유자가 존재하지 않는 파일 및 디렉토리 존재 여부 점검" >> $OUTPUT_FILE
echo "==============================================================" >> $OUTPUT_FILE

if [ -z $U15_FIND ]; then
    echo "[양호] 소유자가 존재하지 않는 파일 또는 디렉토리가 존재하지 않습니다." >> $OUTPUT_FILE
else
    echo "[취약] 소유자가 존재하지 않는 파일 또는 디렉토리가 존재합니다." >> $OUTPUT_FILE
    echo "====================================================================================================" >> $OUTPUT_FILE
    echo -e "발견된 디렉토리 및 파일\n\n$U15_FIND" | head -n 7 >> $OUTPUT_FILE
    echo "====================================================================================================" >> $OUTPUT_FILE
fi
...
echo -e "\n==============================================================" >> $OUTPUT_FILE
echo "U19.hosts파일 소유자 및 권한 점검" >> $OUTPUT_FILE
echo "==============================================================" >> $OUTPUT_FILE

U19_PMS=$(stat -c "%a" /etc/hosts)
U19_HOSTS="/etc/hosts"

echo -e "\n----------------------------------------------" >> $OUTPUT_FILE
echo "점검 진행 파일 : $U19_HOSTS" >> $OUTPUT_FILE
echo "----------------------------------------------" >> $OUTPUT_FILE

if [ "$U19_PMS" -le 644 ]; then
    echo "[양호] hosts파일 권한이 정책에 맞게 설정되어 있습니다." >> $OUTPUT_FILE
else
    echo "[취약] hosts파일 권한이 정책에 위반됩니다." >> $OUTPUT_FILE
    echo "권장 설정 값 : 권한 644이하" >> $OUTPUT_FILE
    echo "현재 설정 값 : $U19_PMS" >> $OUTPUT_FILE
fi
